...
class PlaceSerializer(serializers.ModelSerializer):
    ...
    def validate_external_id(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError("Art institute API is currently unavailable. Please try again later.")
        return value
...
class TravelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Travel
        fields = ['id', 'name', 'destination', 'start_date', 'is_completed', 'places']

    def create(self, validated_data):
        places_data = self.context['request'].data.get('places', [])
        
        if places_data and (len(places_data) < 1 or len(places_data) > 10):
            raise serializers.ValidationError("You must provide between 1 and 10 places.")
