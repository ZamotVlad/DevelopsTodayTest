...
class TravelViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.places.filter(is_visited=True).exists():
            return Response({"error": "Cannot delete a travel with unvisited places."}, status=status.HTTP_400_BAD_REQUEST)
...
class PlaceViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if travel.places.count() >= 10:
            return Response({"error": "A travel cannot have more than 10 places."}, status=status.HTTP_400_BAD_REQUEST)
